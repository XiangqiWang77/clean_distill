...
[[ "$CSD_DISTILLATION_STEPS" =~ ^[0-9]+$ ]] && (( CSD_DISTILLATION_STEPS >= 1 ))
[[ "$CSD_TRAIN_MAX_NEW_TOKENS" =~ ^[0-9]+$ ]]
[[ "$CSD_LONG_HORIZON_MIN_PREFIX_TOKENS" =~ ^[0-9]+$ ]]
(( CSD_TRAIN_MAX_NEW_TOKENS >= CSD_LONG_HORIZON_MIN_PREFIX_TOKENS ))
(( CSD_LONG_HORIZON_MIN_PREFIX_TOKENS >= 4096 ))
(( CSD_TRAIN_MAX_NEW_TOKENS <= CSD_EVAL_MAX_NEW_TOKENS ))
[[ "$CSD_TARGET_HORIZON_SPLIT_TOKENS" =~ ^[0-9]+$ ]]
[[ "$CSD_LATE_WINDOW_START_TOKENS" =~ ^[0-9]+$ ]]
[[ "$CSD_MIN_HELDOUT_LATE_QUERIES" =~ ^[0-9]+$ ]]
(( CSD_TARGET_HORIZON_SPLIT_TOKENS == 2048 ))
(( CSD_LATE_WINDOW_START_TOKENS == 2048 ))
(( CSD_MIN_HELDOUT_LATE_QUERIES >= 1 ))
...
CSD_CONFIG_TMP="${CSD_RUN_CONFIG}.tmp.$$"
{
  for CSD_NAME in \
    ...
    CSD_EVAL_MAX_NEW_TOKENS CSD_DISTILLATION_STEPS CSD_TRAIN_MAX_NEW_TOKENS \
    CSD_LONG_HORIZON_MIN_PREFIX_TOKENS CSD_TARGET_HORIZON_SPLIT_TOKENS \
    CSD_LATE_WINDOW_START_TOKENS CSD_MIN_HELDOUT_LATE_QUERIES \
    CSD_PROPOSAL_OVERSAMPLE CSD_PROPOSAL_MAX_ROUNDS \
