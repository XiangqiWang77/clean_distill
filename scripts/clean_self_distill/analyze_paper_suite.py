...
def collect_detail(root: Path, pattern: str, method: str) -> list[dict[str, Any]]:
    ...
    for path in root.glob(pattern):
        ...
        try:
            model = next(part for part in parts if part.startswith("qwen") or part.startswith("olmo"))
        except StopIteration:
            model = "unknown"
        seed_part = next((part for part in parts if part.startswith("seed_")), "seed_0")
        for row in read_jsonl(path):
            result.append({**row, "model": model, "seed": int(seed_part.split("_", 1)[1]), "method": method})
    return result
...
def load_suite(root: Path) -> dict[str, list[dict[str, Any]]]:
    return {
        "task1": collect_detail(root, "main/*/seed_*/csd_t/eval_task1_fast_teacher.jsonl", "CSD-T"),
        "task2": collect_detail(
            root,
            "main/*/seed_*/csd_sd/eval_task2_clean_distillation.jsonl",
            "CSD-SD",
        ),
        "icl": collect_detail(root, "main/*/seed_*/support_icl/eval_support_icl.jsonl", "Support ICL"),
        "head_sgd": collect_detail(root, "main/*/seed_*/head_sgd/eval_head_sgd.jsonl", "Head SGD"),
        ...
        "budget": collect_detail(root, "budget/*/seed_*/*/eval_task1_fast_teacher.jsonl", "budget"),
        "hindsight": collect_detail(root, "hindsight/*/seed_*/eval_task1_fast_teacher.jsonl", "hindsight"),
        ...
        "ood_t": collect_detail(root, "ood/*/*/seed_*/csd_t/eval_task1_fast_teacher.jsonl", "CSD-T"),
                ...
                result.append(
                    {
                        ...
                        "verifier_valid": float(bool(candidate.get("verifier_valid", False))),
                        "literal_overlap_rate": float(audit.get("literal_overlap_rate", 0.0)),
                        "fourgram_overlap_rate": float(audit.get("fourgram_overlap_rate", 0.0)),
                        "literal_overlap_count": float(audit.get("literal_overlap_count", 0.0)),
                        "fourgram_overlap_count": float(audit.get("fourgram_overlap_count", 0.0)),
                    }
                )
    return result


def build_tables(root: Path, analysis_dir: Path, suite: dict[str, list[dict[str, Any]]], config: dict[str, Any]) -> None:
    main_long = []
    for row in suite["task1"]:
        ...
        if "privileged_correct" in row:
            main_long.append({**row, "method": "Privileged control", "score": float(row["privileged_correct"])})
    main_long.extend({**row, "score": float(row["distilled_correct"])} for row in suite["task2"])
    main_long.extend({**row, "score": float(row["correct"])} for row in suite["icl"])
    main_long.extend({**row, "score": float(row["correct"])} for row in suite["head_sgd"])
    main_long.extend({**row, "score": float(row["correct"])} for row in suite["support_lora"])
    main_long.extend({**row, "score": float(row["base_majority_at_n"])} for row in suite["sc"])
    ...
    for path in root.glob("budget/*/seed_*/samples_*/eval_task1_fast_teacher.jsonl"):
        parts = path.relative_to(root).parts
        model, seed, samples = parts[1], int(parts[2].split("_")[1]), int(parts[3].split("_")[1])
        for row in read_jsonl(path):
                ...
                [
                    {
                        ...
                        "generated_tokens": float(row.get("base_generated_tokens", 0.0)),
                    },
                    {
                        "model": model,
                        "seed": seed,
                        ...
                        "accuracy": float(row.get("base_majority_at_n", row["base_correct"])),
                        "generated_tokens": float(row.get("base_generated_tokens", 0.0)),
                    },
                    {
                        "model": model,
                    ...
                    },
                    {
                        ...
                        "accuracy": float(row.get("teacher_majority_at_n", row["teacher_correct"])),
                        "generated_tokens": support_tokens
                        + float(row.get("teacher_generated_tokens", 0.0)),
                    },
                ]
    ...
    token_map = {(row["model"], row["samples"], row["method"]): row["value"] for row in token_groups}
    for row in table_budget:
        row["mean_generated_tokens"] = token_map[(row["model"], row["samples"], row["method"])]
        ...
        table_support.append(
            {
                ...
                "literal_overlap_rate": mean(row["literal_overlap_rate"] for row in group),
                "fourgram_overlap_rate": mean(row["fourgram_overlap_rate"] for row in group),
            }
        )
    ...
    for model in sorted({row["model"] for row in suite["task1"]}):
        ...
        positive_gain = sum(max(float(row["target_answer_nll_gain"]), 0.0) for row in t_rows)
        total_time = sum(float(row["specialization_seconds"]) for row in t_rows)
        teacher_table.append(
            {
                "model": model,
                "method": "CSD-T",
                "target_nll_gain": mean(float(row["target_answer_nll_gain"]) for row in t_rows),
                "specialization_success_rate": mean(
                    float(row["target_answer_nll_gain"] > 0) for row in t_rows
                ),
                "clean_accuracy_gain": mean(
                    float(row["teacher_correct"]) - float(row["base_correct"]) for row in t_rows
                ),
                "HER": mean(float(row.get("hindsight_exposure_rate", 0.0)) for row in t_rows),
                "CAR": mean(float(row.get("clean_advantage_retention", 0.0)) for row in t_rows),
                "CHS": 0.0,
        ...
        for suite_key, label in (("head_sgd", "Head SGD"), ("support_lora", "Support LoRA")):
            ...
            baseline_time = sum(float(row["adaptation_seconds"]) for row in baseline_rows)
            teacher_table.append(
                {
                    "model": model,
                    "method": label,
                    ...
                    "specialization_success_rate": mean(
                        float(row["target_answer_nll_gain"] > 0) for row in baseline_rows
                    ),
                    "clean_accuracy_gain": mean(
                    ...
                    "peak_memory_bytes": mean(
                        float(row.get("peak_memory_bytes", 0.0)) for row in baseline_rows
                    ),
        ...
        if privileged_rows:
            teacher_table.append(
                {
                    "model": model,
                    "method": "Privileged hint control",
                    ...
                    "CAR": 0.0,
                    "CHS": mean(
                        float(row.get("privileged_counterfactual_jsd", 0.0))
                        for row in privileged_rows
                    ),
                    "answer_flip_rate": mean(
                        float(row.get("privileged_answer_flip_rate", 0.0))
                        for row in privileged_rows
                    ),
                    "CPP": 0.0,
                    "accuracy_teacher_gain_retention": float("nan"),
    ...
    write_csv(
        ...
        [row for row in teacher_table if row["method"] != "Privileged hint control"],
    )

    sensitivity_table = []
    for path in root.glob("sensitivity/*/seed_*/*/*/eval_task1_fast_teacher.jsonl"):
        parts = path.relative_to(root).parts
        model, seed, sweep, value = parts[1], int(parts[2].split("_")[1]), parts[3], parts[4]
        ...
        sensitivity_table.append(
            {
                ...
                "target_nll_gain": mean(float(row["target_answer_nll_gain"]) for row in rows),
                "adaptation_seconds": mean(float(row["specialization_seconds"]) for row in rows),
            }
        )
    write_csv(analysis_dir / "tables/table6_sensitivity_ablation.csv", sensitivity_table)
    ...
    for method_key, rows in (("CSD-T", suite["ood_t"]), ("CSD-SD", suite["ood_sd"])):
        for row in rows:
            score = row["teacher_correct"] if method_key == "CSD-T" else row["distilled_correct"]
    ...
    write_csv(
        ...
        grouped_rows(ood_records, ("model", "method", "source"), lambda row: row["score"]),
    )
        ...
        if isinstance(values, dict):
            for name, value in values.items():
                if not isinstance(value, dict):
                    hyper_rows.append({"group": group, "hyperparameter": name, "value": value})
        ...
        cost_rows.append(
            {
                ...
                "seed": row["seed"],
                "source": row["source"],
                "support_generation_seconds": row.get("support_generation_seconds", 0.0),
                "feature_extraction_seconds": row.get("feature_extraction_seconds", 0.0),
...
def plot_figures(root: Path, analysis_dir: Path, suite: dict[str, list[dict[str, Any]]]) -> list[str]:
    try:
        import matplotlib.pyplot as plt
        import numpy as np
    except ImportError as exc:
        raise ImportError("Figure generation requires matplotlib (pip install matplotlib).") from exc
    ...
    if supports:
        ...
        ax.set(xlabel="target literal overlap (%)", ylabel="target 4-gram overlap (%)", title="Support hygiene")
        path = figure_dir / "fig2_support_hygiene.pdf"
        fig.tight_layout(); fig.savefig(path); plt.close(fig); generated.append(str(path))

    if suite["task1"]:
        fig, ax = plt.subplots(figsize=(5.4, 3.3))
        x = [float(row.get("proposal_fit_target_logit_gain", 0.0)) for row in suite["task1"]]
        y = [float(row["target_answer_nll_gain"]) for row in suite["task1"]]
        ax.scatter(x, y, s=13, alpha=0.35)
        ax.axhline(0, color="black", lw=0.8, ls="--")
        ax.set(xlabel="proposal-fit target-logit gain", ylabel="post-hoc target NLL gain", title="Fit signal vs target transfer")
        ...
        ax.set(xlabel="specialization seconds/query", ylabel="target NLL gain", title="Fast-teacher efficiency frontier")
        ax.set_xscale("log")
    ...
    for path in root.glob("budget/*/seed_*/samples_*/eval_task1_fast_teacher.jsonl"):
        ...
        budget_points.extend(
            [
                {
                    "samples": samples,
                    "tokens": mean(float(row.get("base_generated_tokens", 0.0)) for row in rows),
                    "label": f"{model} / Base",
                    "score": mean(
    ...
    if budget_points:
        ...
        ax.set(xlabel="total generated tokens/query", ylabel="majority accuracy", title="Accuracy--budget frontier")
    ...
    if hindsight:
        ...
        clean_gain = [mean(float(r["teacher_correct"]) - float(r["base_correct"]) for r in hindsight if r["model"] == m) for m in models]
        privileged_gain = [mean(float(r["privileged_correct"]) - float(r["base_correct"]) for r in hindsight if r["model"] == m) for m in models]
        car = [mean(float(r.get("clean_advantage_retention", 0.0)) for r in hindsight if r["model"] == m) for m in models]
        chs = [mean(float(r.get("privileged_counterfactual_jsd", 0.0)) for r in hindsight if r["model"] == m) for m in models]
        flips = [mean(float(r.get("privileged_answer_flip_rate", 0.0)) for r in hindsight if r["model"] == m) for m in models]
        ...
        axes[0].bar(x + width / 2, privileged_gain, width, label="privileged hint")
        axes[0].set_ylabel("accuracy gain"); axes[0].set_title("Utility")
        axes[0].legend(frameon=False, fontsize=7)
        axes[1].bar(x, car, color="#2D8B57")
        axes[1].set_ylim(0, 1.05); axes[1].set_ylabel("CAR"); axes[1].set_title("Clean advantage retention")
        axes[2].bar(x - width / 2, chs, width, label="CHS (JSD)")
        axes[2].bar(x + width / 2, flips, width, label="answer flip")
        axes[2].set_title("Counterfactual sensitivity"); axes[2].legend(frameon=False, fontsize=7)
        for ax in axes:
    ...
    if suite["task1"]:
        values = sorted(suite["task1"], key=lambda row: float(row.get("proposal_fit_target_logit_gain", 0.0)))
        bins = np.array_split(values, min(10, len(values)))
        xs = [mean(float(row.get("proposal_fit_target_logit_gain", 0.0)) for row in group) for group in bins if len(group)]
        ys = [mean(float(float(row["target_answer_nll_gain"]) > 0) for row in group) for group in bins if len(group)]
        fig, ax = plt.subplots(figsize=(5.4, 3.3))
        ax.plot(xs, ys, marker="o")
        ax.set_ylim(-0.03, 1.03)
        ax.set(xlabel="proposal-fit signal (binned)", ylabel="Pr(target NLL improves)", title="Specialization reliability without a runtime gate")
        path = figure_dir / "fig7_transfer_reliability.pdf"
        legacy = figure_dir / "fig7_gate_calibration.pdf"
        fig.tight_layout(); fig.savefig(path); fig.savefig(legacy); plt.close(fig); generated.extend([str(path), str(legacy)])
    ...
    for path in root.glob("sensitivity/*/seed_*/*/*/eval_task1_fast_teacher.jsonl"):
        ...
        rows = read_jsonl(path)
        sensitivity_data[sweep].append((value, mean(float(row["teacher_correct"]) for row in rows)))
    if transfer_points:
        sensitivity_data["distill_steps"].extend(
            (step, mean(point[1] for point in transfer_points if point[0] == step)) for step in sorted({p[0] for p in transfer_points})
        )
    if sensitivity_data:
        fig, axes = plt.subplots(2, 2, figsize=(7.2, 5.2))
        for ax, sweep in zip(axes.flat, ("ridge_lambda", "support_tokens", "support_count", "distill_steps")):
            points = sensitivity_data.get(sweep, [])
            grouped = defaultdict(list)
            for value, score in points:
                grouped[value].append(score)
            xs = sorted(grouped); ys = [mean(grouped[x]) for x in xs]
            ax.plot(xs, ys, marker="o")
            if sweep == "ridge_lambda" and xs:
                ax.set_xscale("log")
            ax.set_title(sweep.replace("_", " ")); ax.set_ylabel("accuracy")
...
def main() -> None:
    ...
    analysis_dir = Path(args.analysis_dir).resolve() if args.analysis_dir else root / "analysis"
