#!/usr/bin/env python3
"""Collect one label-free, post-hoc TRSD trust-region mechanism trajectory."""
...
def _validate_final_trsd_checkpoint(
    ...
    if mismatches:
        raise TrustRegionMechanismError(
            f"Checkpoint is not the requested final TRSD adapter: {mismatches}"
        )
    try:
    ...
    current_method = method_id == "trsd:exponential_teacher_projection"
    historical_method = method_id.endswith(":trust_region")
    if not current_method and not historical_method:
        raise TrustRegionMechanismError("Checkpoint method is not a TRSD trust region")
    ...
    if not isinstance(arguments, dict):
        raise TrustRegionMechanismError("TRSD run manifest lacks arguments")
    if (
        ...
        raise TrustRegionMechanismError(
            "Adapter disagrees with its clean-branch TRSD run manifest"
        )
    ...
    if not completed_final and not historical_rolling_36:
        raise TrustRegionMechanismError(
            "Checkpoint is neither a completed final TRSD checkpoint nor the "
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.base_only:
        ...
        checkpoint = _validate_final_trsd_checkpoint(
