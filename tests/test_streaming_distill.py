...
class StreamingDistillationTests(unittest.TestCase):
    def test_default_objective_is_projected_teacher_to_student_forward_kl(self):
        student_logits = torch.tensor(
            [[[2.0, -0.5, 0.25]]], dtype=torch.float32, requires_grad=True
        )
        teacher_logits = torch.tensor(
            [[[-1.0, 1.75, 0.5]]], dtype=torch.float32, requires_grad=True
        )
        ...
        torch.testing.assert_close(per_token, expected_forward)
        torch.testing.assert_close(loss, expected_forward.mean())
        self.assertFalse(torch.allclose(per_token, expected_reverse))
        loss.backward()
        self.assertIsNotNone(student_logits.grad)
        expected_gradient = student_log.exp() - teacher_log.exp()
        torch.testing.assert_close(student_logits.grad, expected_gradient)
        self.assertIsNone(teacher_logits.grad)

    def test_reverse_kl_is_an_explicit_legacy_option(self):
        student_logits = torch.tensor(
            [[[2.0, -0.5, 0.25]]], dtype=torch.float32, requires_grad=True
        )
        teacher_logits = torch.tensor([[[-1.0, 1.75, 0.5]]], dtype=torch.float32)
        loss, per_token = _same_prefix_distillation_terms(
            student_logits,
            teacher_logits,
            top_k=3,
