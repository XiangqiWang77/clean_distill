...
This bundle reports only completed, auditable artifacts. The sole public performance metric is **strict Acc@1 over the full denominator**: a response must be correct and finish within the fixed 10,240-token generation budget; otherwise it is wrong. No alternative accuracy denominator is emitted. TRSD-16 is deliberately left blank and excluded from every inferential comparison.
...
All four reported checkpoints use Qwen3-8B, the same 143 held-out questions, the same explicit generation-budget prompt, identical deterministic query-specific seeds, temperature 0.6, top-p 0.95, top-k 20, and a 10,240-token cap. Dataset labels are sealed during training and used only by the offline scorer.

The largest endpoint is TRSD-64 at **102/143 (71.33%)**, which is +17.48 pp over Base. It improves all three datasets: AMC23 84.34%, AIME24 63.33%, and AIME25 43.33%.
...
| C7 | TRSD is operationally stable through this observed 64-episode run. | partially_supported | This is not a multi-seed, AULC, or general long-term-stability claim; T16 is intentionally blank. |
...
| L3 | medium | TRSD-16 is intentionally unreported. | No T16-to-T64 long-horizon curve, AULC, or formal clean/privilege crossover is available. |
...
- **Evaluation completeness:** T16, AULC, Mean@4, and fully training-matched P64/T64 ablations remain unreported.
