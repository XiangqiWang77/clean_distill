...
**统一评测已得到 vanilla 基线：111/143（77.62%）。四个训练后 checkpoint 尚未完成评测，不能宣称 OPSD 或 LGSD 提升。** 最新数据见 [统一评测结果](EVALUATION_RESULTS.md)。 下表记录训练过程；长度截断不是准确率指标，蒸馏 loss 也不能直接跨方法比较。
