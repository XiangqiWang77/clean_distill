...
**统一评测的最新准确率、改善和退步明细见 [评测结果](EVALUATION_RESULTS.md)。** 下表记录训练过程；长度截断不是准确率指标，蒸馏 loss 也不能直接跨方法比较。
...
评测作业 `25987666` 已完成：vanilla、OPSD-16、LGSD-16、OPSD-64、LGSD-64 的五组统一 student-only 评测。评测集为事先准备的 143 道 AMC/AIME 题，与 DeepMath 训练题无交集；每题一次配对采样，seed 20260809，生成上限 10,240，所有组启用 repetition penalty 1.1。

评测使用同一 vLLM MXFP4 Marlin 推理配置（所有引擎启用 LoRA 支持，vanilla 不加载 adapter），固定 medium reasoning 和日期，采用 full-response Math-Verify 0.9.0 离线评分。vanilla 会重新运行，避免把旧解码设置的成绩混进本轮比较。已保留相对 vanilla 救回／损失的题目和截断率；单 seed 对比不作为显著性证明。
