...
**A concrete same-prefix example.** Consider one held-out DeepMath trajectory
where the student has already derived the residue $1/24$ but continues,
“Alternatively, maybe I can use the fact that …”. At that exact prefix, the raw
privileged target changes the realized-token log-probability of
“Alternatively” by $-2.969$, so its probability is only
$\exp(-2.969)=0.051\times$ the ordinary student's. With the diagnostic
$\varepsilon=0.008$ projection ($\alpha=0.675$), the change is $-1.915$, or
$0.147\times$ the student's probability: a 35.5% contraction in absolute
log-probability shift. The largest distinct style-lexicon matches on this
trajectory show the same pattern:

| Realized style token | Raw privileged $\Delta\log p$ | Projected $\Delta\log p$ | Absolute shift contraction |
|---|---:|---:|---:|
| Alternatively | -2.969 | -1.915 | 35.5% |
| indeed | -0.915 | -0.583 | 36.3% |
| perhaps | +0.316 | +0.232 | 26.6% |
| Therefore | -0.222 | -0.141 | 36.2% |
| Suppose | +0.201 | +0.146 | 27.3% |

Here $\Delta\log p=\log q_t(y_t)-\log p_t(y_t)$; its sign indicates whether
the target raises or lowers the realized token, whereas StyleDrift uses its
absolute value. Averaged over all 44 style-token positions in the same neutral
trajectory, projection reduces StyleDrift from $0.2273$ to $0.1428$ and mean
full-vocabulary target KL from $0.02035$ to $0.00800$. This is a deliberately
local mechanism illustration—not an accuracy result—and uses the post-hoc
stress-test budget described in Study 3. [Top-5 token CSV](docs/results/style_drift_token_example_query_0022.csv).
