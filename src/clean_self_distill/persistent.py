...
class PersistentConfig:
    ...
    student_kl_direction: str = "reverse"
    ...
    def validate(self) -> None:
        ...
        if self.update_guard and self.branch != "clean":
            raise PersistentProtocolError("update_guard is defined only for TRSD")
        if self.branch != "clean" and (
            self.projection_scope != "trajectory"
            or self.projection_path != "exponential"
            or self.student_kl_direction != "reverse"
            or not self.same_prefix_scoring
        ):
            raise PersistentProtocolError(
                "projection ablations are defined only for TRSD"
            )
    ...
    def distillation_kl_direction(self) -> str:
        if self.student_kl_direction == "forward":
            return "projected_teacher_to_student_forward_kl_v1"
        return DISTILLATION_KL_DIRECTION
    ...
    def identity_payload(self) -> dict[str, Any]:
        ...
        # Keep the identity of the completed reference TRSD/Privilege-SD runs
        # byte-for-byte stable while making every non-default ablation explicit.
        defaults = {
